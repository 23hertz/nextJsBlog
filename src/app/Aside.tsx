...
//   // 🔹 Get 5 random posts
//   const getRandomPosts = (arr: PostItemProps[], count: number = 5) => {
//     if (arr.length <= count) return arr;
//     const shuffled = [...arr].sort(() => 0.5 - Math.random());
//     return shuffled.slice(0, count);
//   };

//   const randomPosts = getRandomPosts(allPosts, 5);
...
//   return (
//     <div className="mt-8 p-4 rounded-lg shadow-lg bg-gray">
//       <header>
//         <h3 className="text-center text-xl font-bold mb-4">Staff Picks</h3>
//       </header>
...
const Aside = () => {
  const [randomPosts, setRandomPosts] = useState<PostItemProps[]>([]);
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, "flashblog"), (snapshot) => {
      ...
      // 🔹 Immediately pick 5 random posts
